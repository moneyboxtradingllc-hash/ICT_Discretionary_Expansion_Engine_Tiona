...
def _print_snapshot_breakdown(snap: dict):
    ...
    ai_disc = snap.get("ai_discretionary", {})
    fusion  = snap.get("confidence_fusion", {})

    print()
    _divider()
    print("AI DISCRETIONARY ENGINE (Phase 1M)")
    _divider()

    if not ai_disc:
        print("  No AI discretionary output available.")
    else:
        ai_mode       = ai_disc.get("ai_mode",               "unknown")
        provider      = ai_disc.get("provider",               None)
        model         = ai_disc.get("model",                  None)
        ext_conn      = ai_disc.get("external_ai_connected",  False)
        fallback_used = ai_disc.get("fallback_used",          False)
        fallback_why  = ai_disc.get("fallback_reason",        None)
        latency_ms    = ai_disc.get("latency_ms",             None)
        direction     = ai_disc.get("ai_direction",           "?")
        ...
        fstatus       = fusion.get("fusion_status",       "?")

        print(f"  Mode             : {ai_mode}")
        print(f"  Provider         : {provider if provider else 'None'}")
        print(f"  Model            : {model if model else 'None (internal rule-based engine)'}")
        print(f"  External AI      : {'Connected' if ext_conn else 'Not connected'}")
        print(f"  Fallback Used    : {'Yes' if fallback_used else 'No'}")
        print(f"  Fallback Reason  : {fallback_why if fallback_why else 'None'}")
        print(f"  Latency          : {f'{latency_ms} ms' if latency_ms is not None else 'None'}")
        print()
        print(f"  Direction        : {direction.upper()}")
        print(f"  AI Confidence    : {confidence}/100")
        print(f"  Agree w/Playbook : {'Yes' if agree_pb else 'NO - disagrees'}")
        print(f"  Agree w/Risk     : {'Yes' if agree_risk else 'NO - disagrees'}")
        print()

        print(f"  Confidence Fusion:")
        print(f"    Mechanical     : {mech}")
        print(f"    AI             : {confidence}")
        print(f"    Combined       : {combined}")
        print(f"    Status         : {fstatus.upper()}")
        print()

        story = ai_disc.get("market_story", "")
        if story:
        ...
        inv = ai_disc.get("invalidation_thesis", "")
        if inv:
            print(f"  Invalidation Thesis:")
            words = inv.split()
            line = "    "
            for w in words:
                if len(line) + len(w) + 1 > 80:
        ...
        pref_sc = ai_disc.get("preferred_scenario", "")
        alt_sc  = ai_disc.get("alternative_scenario", "")
        if pref_sc:
            print(f"  Preferred Scenario:")
            words = pref_sc.split()
            line = "    "
            for w in words:
                if len(line) + len(w) + 1 > 80:
