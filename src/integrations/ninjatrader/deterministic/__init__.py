...
Frozen doctrine (this lane):
  ...
  * Quantity: EXACTLY 15 contracts (15 or no trade — never auto-reduced)
  * Target  : fixed 35.00 points from the actual average fill
  * Stop    : the setup's STRUCTURAL invalidation; HARD cap 16.50 points
...
Risk basis: MNQ is $2/point/contract, so 15 contracts = $30/point. The 16.50pt
stop cap holds max per-trade risk to $495 (< the $1000 daily ceiling, which
covers the two configured trades/day at up to $990 total).
"""
...
QUANTITY = 15                    # exactly fifteen — never more, never fewer
POINT_VALUE = 2.00              # $ per index point per contract
DOLLARS_PER_POINT = POINT_VALUE * QUANTITY   # $30.00 per point at 15 contracts
TICK_SIZE = 0.25

TARGET_POINTS = 35.00           # fixed profit distance
MAX_STOP_POINTS = 16.50         # HARD structural-stop cap (holds risk to $495)
MAX_GROSS_TRADE_RISK = MAX_STOP_POINTS * DOLLARS_PER_POINT   # $495
TARGET_GROSS_REWARD = TARGET_POINTS * DOLLARS_PER_POINT      # $1050
MIN_RR_AT_MAX_STOP = TARGET_POINTS / MAX_STOP_POINTS         # 2.1212
