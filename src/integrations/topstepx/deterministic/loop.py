...
from integrations.topstepx.deterministic import risk as RISK
from integrations.topstepx.deterministic import facts_provider as FP
from integrations.topstepx.deterministic import evidence as EV
from integrations.topstepx.deterministic import htf_accum as HTF_ACCUM
from integrations.topstepx.deterministic.funnel import funnel_console
from integrations.topstepx.deterministic.session import SessionAuthority, ACTIVE, STOPPED_MANUAL

STOP_FILE = os.path.join("data", "integration", "topstepx", "deterministic", "STOP")
...
def _venue_client():
    ...
    # TOPSTEPX IS THE ONLY ROUTE. The NinjaTrader fall-through was deleted with
    # the venue itself; anything else is a configuration error, not a lane.
    raise RuntimeError(
        f"DETERMINISTIC_VENUE={VENUE!r} is not a venue this lane can route to. "
        f"TopstepX is the only supported brokerage; use 'topstepx'.")
...
def one_scan(client, session: SessionAuthority, scan_num: int) -> dict:
    ...
    # HTF-MNQ-ACCUM — passively fold this scan's real bars into the multi-day
    # store (data/htf_memory/MNQ.json). WRITE-ONLY: the author/facts/risk path
    # never reads it (test-locked); memory_age below is telemetry, not input.
    _htf_ctx = HTF_ACCUM.accumulate(bars)
    ...
    scan_record = {
        ...
        "snapshot": {"bars": len(bars), "last": quote.get("last"),
                     # HTF-MNQ-ACCUM telemetry (write-only; no reader)
                     "htf_memory_age": (_htf_ctx or {}).get("memory_age"),
...
def run(max_scans: Optional[int] = None):
    ...
    if VENUE == "topstepx":
        from integrations.topstepx.deterministic.topstepx_lane_client import _armed
