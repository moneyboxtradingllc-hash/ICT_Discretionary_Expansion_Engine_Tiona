...
from integrations.topstepx.deterministic.funnel import funnel_trace
...
def build_facts_from_snapshot(snapshot: dict, decision: dict, gate: dict,
    ...
    # CONTINUITY-2F — a PROVISIONAL zone may not author a stop, and refusing it
    # must not fall through to the structural fallback below. That fallback is
    # for a level that does not exist; this level exists and is ineligible, and
    # quietly swapping in a different one would be exactly the substitution 2F
    # forbids. No invalidation, no order.
    pl_ineligible = pl.get("execution_eligible") is False
    if pl_ineligible:
        invalidation = None
    elif not isinstance(invalidation, (int, float)):
