...
Frozen doctrine (this lane):
  * Account : from TOPSTEPX_ACCOUNT_NAME (.env)
  * Instrument : from TOPSTEPX_CONTRACT
...
#
# THE IDENTITY IS OPERATOR-SUPPLIED. There is no default: a placeholder
# would be compared against the real TopstepX account name, come back
# False on every scan, and the fail-closed author would refuse every
# trade while looking like a quiet market. Absent config must therefore
# raise here, loudly, rather than degrade a gate into 'unknown'.
#
VENUE = os.getenv("DETERMINISTIC_VENUE", "topstepx").strip().lower()

# LUNA-TOPSTEPX-ONLY (2026-08-31). NinjaTrader and Alpaca were removed as
# supported venues. This lane historically served both, so it branched on
# VENUE and imported a NinjaTrader bridge at module scope. There is now ONE
# venue, and a venue that no longer exists must refuse loudly rather than
# fall through to a half-present code path.
if VENUE != "topstepx":
    raise RuntimeError(
        f"Unsupported venue {VENUE!r}: TopstepX is the only supported "
        f"brokerage. NinjaTrader and Alpaca support was removed "
        f"deliberately and there is no fallback lane to select. "
        f"Set DETERMINISTIC_VENUE=topstepx.")

# The TopstepX adapter resolves this name by EXACT match, refuses an
# ambiguous match, refuses a non-simulated account without an explicit
# opt-in, and refuses one Topstep has disabled -- all before a bar is read.
ACCOUNT = os.getenv("TOPSTEPX_ACCOUNT_NAME", "").strip()
INSTRUMENT = os.getenv("TOPSTEPX_CONTRACT", "").strip()
_REQUIRED = (("TOPSTEPX_ACCOUNT_NAME", ACCOUNT),
             ("TOPSTEPX_CONTRACT", INSTRUMENT))
...
if not ACCOUNT or not INSTRUMENT:
    ...
    raise RuntimeError(
        ...
        f"Defaulting these would point your bot at another operator's "
        f"account, so there is deliberately no default.")
